...
class MultimodalGenerativeCVAE(object):
    ...
    def obtain_encoded_tensors(
        ...
        memory_src_mask = generate_square_subsequent_mask(
            node_history_st.size()[-2], self.device)
        transformer_encoder = self.node_modules[self.node_type +
                                                "/node_history_encoder"]
        memory = transformer_encoder(
            node_history_st, memory_src_mask, memory_padding_mask)
        ############################
    ...
    def non_at_dec(
        ...
        transformer_decoder = self.node_modules[self.node_type +
        ...
        if self.hyperparams["lane_cnn_encoding"]:
            ...
            if self.hyperparams["map_cnn_encoding"] or self.hyperparams["map_vit_encoding"]:
                ...
                h_state = fusion(
                    torch.cat([h_state, map_feature.unsqueeze(-2)], dim=-1))
    ...
    def at_dec(
        ...
        if self.hyperparams["lane_cnn_encoding"]:
            ...
            for _ in range(prediction_horizon):
                tgt_mask = generate_square_subsequent_mask(
                    history_input.size()[-2], self.device)
                h_state = transformer_decoder(
                    tgt=history_input,
                    memory=memory,
                    tgt_mask=tgt_mask,
                ...
                if self.hyperparams["map_cnn_encoding"] or self.hyperparams["map_vit_encoding"]:
                    h_state = fusion(torch.cat(
                        [h_state, map_feature.unsqueeze(-2)], dim=-1))
    ...
    def predict(
        ...
        (memory,
         ...
         encoded_map,
                                         ...
                                         map=map)
        history_pred, lane_pred, lane_attn = self.test_decoder(
            True, memory, memory_mask, memory_key_padding_mask, n_s_t0, encoded_lane, encoded_map, prediction_horizon)
        if self.hyperparams["lane_cnn_encoding"]:
            lane_pred = torch.stack(lane_pred, dim=1)
            max_lane_num = lane_pred.size()[1]
            lane_pred = lane_pred * 80 + \
