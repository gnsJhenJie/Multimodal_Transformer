...
def lane_prediction_output_to_trajectories(prediction_output_dict,
                                           dt,
                                           max_h,
                                           ph,
                                           map=None,
    ...
    for t in prediction_timesteps:
        ...
        for node in prediction_nodes:
            ...
            histories_dict[t][node] = dict()
            output_dict[t][node] = dict()
            futures_dict[t][node] = dict()

            for lane in predictions_output.keys():

                lane_predictions_output = predictions_output[lane]

                if prune_ph_to_future:
                    lane_predictions_output = lane_predictions_output[:,
                                                                      :, :future.shape[0]]
                    if predictions_output.shape[2] == 0:
                        continue

                trajectory = lane_predictions_output

                if map is None:
                    histories_dict[t][node][lane] = history
                    output_dict[t][node][lane] = trajectory
                    futures_dict[t][node][lane] = future
                else:
                    histories_dict[t][node][lane] = map.to_map_points(history)
                    output_dict[t][node][lane] = map.to_map_points(trajectory)
                    futures_dict[t][node][lane] = map.to_map_points(future)
