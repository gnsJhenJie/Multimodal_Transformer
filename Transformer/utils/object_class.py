...
class Lane():
    ...
    def get_nearest_point_index(self, cur_point):
        ...
        delta_pos = self.lane_set_points - cur_point # lane set points subtract current pos
        self.nearest_point_index = np.argmin(LA.norm(delta_pos,axis=-1))
        self.nearest_point_index = self.nearest_point_index + 1 if self.nearest_point_index== 0 else self.nearest_point_index
    ...
    def get_lane_vector(self, cur_point):
        ...
        while LA.norm(self.lane_set_points[next_pt] - self.lane_set_points[self.nearest_point_index], axis=-1) <= 1e-5 :
            next_pt = next_pt + 1
        # Get lane vector
        self.lane_vector = self.lane_set_points[next_pt] - self.lane_set_points[self.nearest_point_index]
...
class agent():
    ...
    def get_line(self, lanes_dict, cur_lanes_id, min_distance):
        ...
        nearest_point_index = lanes_dict[closest_id].get_attribute('nearest_point_index')
    ...
    def get_parallel_line(self):
        ...
        a = 1e-10 if self.agent_vector[0] == 0 else self.agent_vector[1] / self.agent_vector[0]
