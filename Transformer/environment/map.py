...
class GeometricMap(Map):
    ...
    def __init__(self, data, homography, description=None):
        ...
        super(GeometricMap, self).__init__(
            data, homography, description=description)
    ...
    def torch_map(self, device):
        ...
        self._torch_map = torch.tensor(
            self.data, dtype=torch.uint8, device=device)
        return self._torch_map
    ...
    def get_padded_map(self, padding_x, padding_y, device):
        if self._last_padding == (padding_x, padding_y):
            ...
            self._last_padded_map = torch.full(
                ...
                False,
                dtype=torch.uint8)
            self._last_padded_map[..., padding_x:-padding_x,
                                  padding_y:-padding_y] = self.torch_map(device)
            return self._last_padded_map
    ...
    def batch_rotate(map_batched, centers, angles, out_height, out_width):
        ...
        rotated_map_batched = warp_affine_crop(
            map_batched, centers, M, dsize=(
                out_height, out_width), padding_mode='zeros')
    ...
    def get_cropped_maps_from_scene_map_batch(
        ...
        centers = torch.tensor(np.array([s_map.to_map_points(scene_pts[np.newaxis,
                                                                       i]) for i,
                                         s_map in enumerate(maps)]),
                               dtype=torch.long,
                               device=device).squeeze(dim=1) + torch.tensor([context_padding_x,
                                                                             context_padding_y],
                                                                            device=device,
                                                                            dtype=torch.long)

        padded_map = [
            s_map.get_padded_map(
                context_padding_x,
                context_padding_y,
                device=device) for s_map in maps]
        ...
        return rotated_map_batched[..., long_size_half -
                                   patch_size[1]:(long_size_half +
                                                  patch_size[3]), lat_size_half -
                                   patch_size[0]:(lat_size_half +
                                                  patch_size[2])]
...
class ImageMap(
        Map):  # TODO Implement for image maps -> watch flipped coordinate system
    def __init__(self):
        raise NotImplementedError
