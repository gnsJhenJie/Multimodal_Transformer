...
class OnlineMultimodalGenerativeCVAE(MultimodalGenerativeCVAE):
    def __init__(self,
        ...
        self.state_length = int(np.sum(
            [len(entity_dims) for entity_dims in self.state[self.node.type].values()]))
        if self.hyperparams['incl_robot_node']:
            self.robot_state_length = int(np.sum(
                [len(entity_dims) for entity_dims in self.state[self.robot.type].values()]))
        ...
        dynamic_class = getattr(
            dynamic_module, self.hyperparams['dynamic'][self.node_type]['name'])
    ...
    def update_graph(self, new_scene_graph, new_neighbors, removed_neighbors):
        ...
        if self.node in new_neighbors:
            for edge_type, new_neighbor_nodes in new_neighbors[self.node].items(
                ...
                self.edge_types += Counter(
                    {edge_type: len(new_neighbor_nodes)})
        ...
        if self.node in removed_neighbors:
            for edge_type, removed_neighbor_nodes in removed_neighbors[self.node].items(
                ...
                self.edge_types -= Counter(
                    {edge_type: len(removed_neighbor_nodes)})
    ...
    def _get_other_node_type_from_edge(self, edge_type_str):
        ...
        return NodeType(
            n2_type_str,
            self.env.node_type_list.index(n2_type_str) + 1)
    ...
    def _get_edge_type_from_str(self, edge_type_str):
        ...
        return (
            NodeType(
                n1_type_str,
                self.env.node_type_list.index(n1_type_str) +
                1),
            NodeType(
                n2_type_str,
                self.env.node_type_list.index(n2_type_str) +
                1))
        ...
        if self.hyperparams['edge_encoding']:
            if len(
                self.scene_graph.get_neighbors(
                    self.node,
                    self._get_other_node_type_from_edge(edge_type))) == 0:
    ...
    def obtain_encoded_tensors(self,
        ...
        if self.hyperparams['edge_encoding']:
            ...
            for edge_type in self.edge_types:
                ...
                # We get all nodes which are connected to the current node for
                # the current timestep
                connected_nodes_batched.append(self.scene_graph.get_neighbors(
                    self.node, self._get_other_node_type_from_edge(edge_type)))

                if self.hyperparams['dynamic_edges'] == 'yes':
                    # We get the edge masks for the current node at the current
                    # timestep
                    edge_masks_for_node = self.scene_graph.get_edge_scaling(
                        self.node)
        ...
        if self.hyperparams['use_map_encoding'] and self.node_type in self.hyperparams['map_encoder']:
            if self.node not in maps:
                # This means the node was removed (it is only being kept around
                # because of the edge removal filter).
                me_params = self.hyperparams['map_encoder'][self.node_type]
                self.TD['encoded_map'] = torch.zeros(
                    (1, me_params['output_size']))
            else:
                encoded_map = self.node_modules[self.node_type + '/map_encoder'](
                    maps[self.node] * 2. - 1., (mode == ModeKeys.TRAIN))
        ...
        if (self.hyperparams['incl_robot_node']
            ...
            if self.hyperparams['edge_encoding']:
                total_edge_influence = TD['total_edge_influence'].repeat(
                    robot_future_st.size()[0], 1)
            if self.hyperparams['use_map_encoding'] and self.node_type in self.hyperparams['map_encoder']:
                encoded_map = TD['encoded_map'].repeat(
                    robot_future_st.size()[0], 1)

        elif self.hyperparams['incl_robot_node'] and self.robot is not None:
            # Four times because we're trying to mimic a bi-directional RNN's
            # output (which is c and h from both ends).
    ...
    def encode_node_history(self, inputs_st):
        ...
        if self.node.type + '/node_history_encoder' not in self.curr_hidden_states:
            outputs, self.curr_hidden_states[self.node.type +
                                             '/node_history_encoder'] = self.node_modules[self.node.type +
                                                                                          '/node_history_encoder'](new_state)
        else:
            outputs, self.curr_hidden_states[self.node.type +
                                             '/node_history_encoder'] = self.node_modules[self.node.type +
                                                                                          '/node_history_encoder'](new_state, self.curr_hidden_states[self.node.type +
    ...
    def encode_edge(
        ...
        for node in connected_nodes[0]:
            ...
            equal_dims = np.min(
                (neighbor_state_np.shape[-1], orig_rel_state.shape[-1]))
            ...
            neighbor_state = torch.tensor(
                neighbor_state_np_st).float().to(self.device)
            neighbor_states.append(neighbor_state)

        if len(
                neighbor_states) == 0:  # There are no neighbors for edge type # TODO necessary?
            neighbor_state_length = int(
                np.sum([len(entity_dims) for entity_dims in self.state[edge_type[1]].values()]))
            edge_states_list.append(torch.zeros(
                (1, 1, neighbor_state_length), device=self.device))
        else:
        ...
        if self.hyperparams['edge_state_combine_method'] == 'sum':
            ...
            if self.hyperparams['dynamic_edges'] == 'yes':
                ...
                for edge_mask in edge_masks:
                    op_applied_edge_mask_list.append(torch.clamp(
                        torch.sum(edge_mask, dim=0, keepdim=True), max=1.))
            ...
            if self.hyperparams['dynamic_edges'] == 'yes':
                ...
                for edge_mask in edge_masks:
                    op_applied_edge_mask_list.append(torch.clamp(
                        torch.max(edge_mask, dim=0, keepdim=True), max=1.))
                combined_edge_masks = torch.stack(
                    op_applied_edge_mask_list, dim=0)
            ...
            if self.hyperparams['dynamic_edges'] == 'yes':
                ...
                for edge_mask in edge_masks:
                    op_applied_edge_mask_list.append(torch.clamp(
                        torch.mean(edge_mask, dim=0, keepdim=True), max=1.))
                combined_edge_masks = torch.stack(
                    op_applied_edge_mask_list, dim=0)
        ...
        if edge_type + '/edge_encoder' not in self.curr_hidden_states:
            outputs, self.curr_hidden_states[edge_type +
                                             '/edge_encoder'] = self.node_modules[edge_type +
                                                                                  '/edge_encoder'](joint_history)
        else:
            outputs, self.curr_hidden_states[edge_type +
                                             '/edge_encoder'] = self.node_modules[edge_type +
                                                                                  '/edge_encoder'](joint_history, self.curr_hidden_states[edge_type +
                                                                                                                                          '/edge_encoder'])
    ...
    # but if it's given then we'll re-run that part of the model (if the node
    # is adjacent to the robot).
