...
class Unicycle(Dynamic):
    ...
    def integrate_samples(self, control_samples, x=None):
        ...
        # In case the input is batched because of the robot in online use we
        # repeat this to match the batch size of x.
        ...
        phi_0 = phi_0 + \
            torch.tanh(self.p0_model(torch.cat((x, phi_0), dim=-1)))

        u = torch.stack(
            [control_samples[..., 0], control_samples[..., 1]], dim=0)
        x = torch.stack([p_0[..., 0], p_0[..., 1], phi_0,
                         torch.norm(v_0, dim=-1)], dim=0).squeeze(dim=-1)
    ...
    def compute_control_jacobian(self, sample_batch_dim, components, x, u):
        ...
        F[..., 0, 0] = ((v /
                        ...
                        torch.cos(phi_p_omega_dt) *
                        self.dt ** 2)
        F[..., 0, 1] = (1 / dphi) * dcos_domega + (1 / dphi) * \
            torch.sin(phi_p_omega_dt) * self.dt
        ...
        F[..., 1, 0] = ((v /
                        ...
                        torch.sin(phi_p_omega_dt) *
                        self.dt ** 2)
        F[..., 1, 1] = (1 / dphi) * dsin_domega - (1 / dphi) * \
            torch.cos(phi_p_omega_dt) * self.dt
    ...
    def compute_jacobian(self, sample_batch_dim, components, x, u):
        ...
        F[..., 0, 2] = v * dcos_domega - \
            (a / dphi) * dsin_domega + (a / dphi) * \
            torch.cos(phi_p_omega_dt) * self.dt
        F[..., 0, 3] = dsin_domega

        F[..., 1, 2] = v * dsin_domega + \
            (a / dphi) * dcos_domega + (a / dphi) * \
            torch.sin(phi_p_omega_dt) * self.dt
        F[..., 1, 3] = -dcos_domega
        ...
        F_sm[..., 0, 2] = -v * \
            torch.sin(phi) * self.dt - (a * torch.sin(phi) * self.dt ** 2) / 2
        F_sm[..., 0, 3] = torch.cos(phi) * self.dt

        F_sm[..., 1, 2] = v * \
            torch.cos(phi) * self.dt + (a * torch.cos(phi) * self.dt ** 2) / 2
    ...
    def integrate_distribution(self, control_dist_dphi_a, x):
        ...
        u = torch.stack([control_dist_dphi_a.mus[..., 0],
                         control_dist_dphi_a.mus[..., 1]], dim=0)
        x = torch.stack([p_0[..., 0], p_0[..., 1], phi_0,
                         torch.norm(v_0, dim=-1)], dim=0)
        ...
        for t in range(ph):
            ...
            pos_dist_sigma_matrix_t = (F_t.matmul(pos_dist_sigma_matrix_t.matmul(
                F_t.transpose(-2, -1))) + G_t.matmul(dist_sigma_matrix_t.matmul(G_t.transpose(-2, -1))))
            pos_dist_sigma_matrix_list.append(
                pos_dist_sigma_matrix_t[..., :2, :2])
