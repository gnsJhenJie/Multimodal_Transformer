...
def process_scene(ns_scene, env, nusc, data_path, lane_process):
    ...
    # store lane information (lane_id -> key, value -> {'next_lane_id':next_id, 'lane_type':lane_type, 'poses':lane_set_point})
    lane_dict = dict()
        ...
        for ns_scene_name in tqdm(ns_scene_names[data_class]):
            ...
            if scene is not None:
                if data_class == 'train':
                    ...
                    for angle in angles:
                        scene.augmented.append(augment_scene(scene, angle, lane_process))
                scenes.append(scene)
...
if __name__ == '__main__':
    ...
    process_data(args.data, args.version, args.output_path, args.val_split, args.lane_process)
